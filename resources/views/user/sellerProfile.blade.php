...
<h1 class="user_text text-center">liked ads</h1>
    ...
    <div class="User_likes column">
                ...
                @foreach (App\Models\Favorite::where('user_id',$user->id)->get() as $favorite)
...
<h1 class="user_text text-center pb-1">I tuoi annunci</h1>
    ...
    <div class="User_likes">           
        ...
        @if(App\Models\Ad::where('user_id',$user->id)->where('is_accepted', 1)->count() == 0)
        <a href="{{route('ad.create')}}"><h5 class="text-white">Nessun annuncio presente. Pubblica ora il tuo primo annuncio!</h5></a>
        @else  
        @foreach (App\Models\Ad::where('user_id',$user->id)->where('is_accepted', 1)->get() as $ad)
        <div class="col-3 responsiv_likes d-flex justify-content-center">
            <div class="ads_container">
    ...
    {{-- <h3 class="user_text text-center pt-5">In fase di revisione:</h3>
        ...
        @if(App\Models\Ad::where('user_id',$user->id)->where('is_accepted', null)->count() == 0)
        <a href="{{route('ad.create')}}"><h5 class="text-white">Nessun annuncio presente. Pubblica ora il tuo primo annuncio!</h5></a>
        @else
        
        @foreach (App\Models\Ad::where('user_id',$user->id)->where('is_accepted', null)->get() as $ad)
        <div class="col-3 responsiv_likes d-flex justify-content-center">
            <div class="ads_container">
